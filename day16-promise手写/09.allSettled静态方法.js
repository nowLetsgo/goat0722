...
//all静态方法
//all方法当里边所有的promise对象都成功的时候，all返回成功，值是所有成功的值组成的数组
//当all中有一个失败，则返回失败的promise对象，值是失败的那个值
MyPromise.all = function (allPromises) {
    return new MyPromise((resolve, reject) => {
        //初始化一个计数器 用来记录成功的数量
        let successCount = 0;
        //初始化一个数组，用来保存成功的值
        let successArray = [];
        //遍历allPromises 一个个的判断状态
        allPromises.forEach((item, index) => {
            //用then检测每一个的状态
            item.then(value => {
                //如果有成功的 则让计数器累加
                successCount++;
                //通过下标的方式放入成功的值，可以保证顺序
                successArray[index] = value;

                //每次判断是否执行完成
                if (successCount === allPromises.length) {
                    //如果完成，说明都成功，则返回成功的promise对象 并且值是successArray
                    resolve(successArray)
                }
            }, reason => {
                //但凡有一个失败，则直接返回失败的promise对象，值失败的值reason
                reject(reason)
            })
        })
    })
...
//allSettled静态方法
//allSettled静态方法永远返回成功的promise对象，值为一个数组，包含所有的promise对象
MyPromise.allSettled = function (allPromises) {
    return new MyPromise((resolve, reject) => {
        //初始化一个计数器 用来记录成功的数量
        let successCount = 0;
        //初始化一个数组，用来保存成功的值
        ...
        //遍历allPromises
        allPromises.forEach((item, index) => {
            //目前只有then可以感知到allPromises中的所有的promise对象状态的改变
            item.then(value => {
                //计数器累加
                successCount++;
                //数组中保存当前的promise对象
                successArray[index] = item;

                //判断是否完成，如果完成则直接返回成功的promise对象
                if (successCount === allPromises.length) {
                    resolve(successArray)
                }
            }, reason => {
